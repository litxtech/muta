...
> = {
  SMALL: { aspectRatio: '16:5', numeric: 16 / 5 },
  MEDIUM: { aspectRatio: '16:6', numeric: 16 / 6 },
  LARGE: { aspectRatio: '16:7', numeric: 16 / 7 },
  HERO: { aspectRatio: '16:9', numeric: 16 / 9 },
};
...
export function parseAspectRatio(ratio: string | null | undefined): number {
  if (!ratio) return BANNER_SIZE_PRESETS.MEDIUM.numeric;
  ...
  return Number.isFinite(n) && n > 0 ? n : BANNER_SIZE_PRESETS.MEDIUM.numeric;
}
...
export function resolveBannerAspect(
  ...
  return BANNER_SIZE_PRESETS[sizeType]?.numeric ?? BANNER_SIZE_PRESETS.MEDIUM.numeric;
}
...
export const BANNER_BORDER_RADIUS = 22;
