...
import { oyunHataKullaniciMesaji, oyunRpcRetryIle } from './OyunAgIstek';
import type {
...
function rpcHata(error: { message?: string } | null): string {
  return oyunHataKullaniciMesaji(error?.message?.trim() || 'Bilinmeyen sunucu hatası');
}
...
export async function createGameSession(
  ...
  try {
    const { data, error } = await oyunRpcRetryIle(() => supabase.rpc('create_game_session', {
    ...
    const hata = oyunHataKullaniciMesaji(e instanceof Error ? e.message : 'create_game_session başarısız');
...
export async function joinGameSession(
  ...
  try {
    const { data, error } = await oyunRpcRetryIle(() => supabase.rpc('join_game_session', {
    ...
    const hata = oyunHataKullaniciMesaji(e instanceof Error ? e.message : 'join_game_session başarısız');
...
export async function startGameSession(
  ...
  try {
    const { data, error } = await oyunRpcRetryIle(() => supabase.rpc('start_game_session', {
    ...
    const hata = oyunHataKullaniciMesaji(e instanceof Error ? e.message : 'start_game_session başarısız');
...
export async function submitGameScore(
  ...
  try {
    const { data, error } = await oyunRpcRetryIle(() => supabase.rpc('submit_game_score', {
      p_session_id: params.sessionId,
    ...
    const hata = oyunHataKullaniciMesaji(e instanceof Error ? e.message : 'submit_game_score başarısız');
...
export async function finishGameSession(
  ...
  try {
    const { data, error } = await oyunRpcRetryIle(() => supabase.rpc('finish_game_session', {
      p_session_id: sessionId,
    }));
    if (error) {
    ...
    const hata = oyunHataKullaniciMesaji(e instanceof Error ? e.message : 'finish_game_session başarısız');
