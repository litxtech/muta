-- Kozmik Kaskad — solo cascade oyun tabloları, config, audit, RPC
-- SERVER = sonuç (Edge Function + bu settle RPC), CLIENT = animasyon
...
-- ---------------------------------------------------------------------------
-- Settle spin — Edge Function (service role) çağırır; sonuç yeniden hesaplanmaz
-- ama edge server-side üretmiş payload'ı atomik yazar.
-- ---------------------------------------------------------------------------
...
begin
  ...
  values (v_round_id, 'settled', jsonb_build_object('bet', p_bet_amount, 'win', v_win));

  insert into public.kaskad_audit_logs (user_id, action, round_id, payload)
  values (p_user_id, 'spin_settled', v_round_id, jsonb_build_object('idempotency', p_idempotency_key));
...
-- service role only in practice (edge); revoke from anon/authenticated
