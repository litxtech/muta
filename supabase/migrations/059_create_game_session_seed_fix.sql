-- create_game_session: gen_random_bytes(integer) does not exist
-- Neden: SECURITY DEFINER + search_path=public iken pgcrypto (extensions.gen_random_bytes) görünmez.
-- Çözüm: PG13+ built-in gen_random_uuid ile seed üret (pg_catalog, search_path'ten bağımsız).
...
begin
  ...
  v_seed := (extract(epoch from now()) * 1000)::bigint
            + (('x' || substr(replace(gen_random_uuid()::text, '-', ''), 1, 4))::bit(16)::int);
