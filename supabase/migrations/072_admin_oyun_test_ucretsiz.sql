-- Admin oyun testi: coin şartı yok (maç giriş + varsa kaskad settle)

-- ---------------------------------------------------------------------------
-- Kozmik Kaskad settle — yalnızca tablolar yüklüyse (065) güncelle
-- ---------------------------------------------------------------------------
do $guard$
begin
  if to_regclass('public.kaskad_rounds') is null then
    raise notice 'kaskad_rounds yok — settle admin muafiyeti atlandı';
    return;
  end if;
...
begin
  ...
  -- Admin denetim: bahis düşmez, kazanç yazılmaz (bakiye sabit)
  if v_is_admin then
    v_debit := 0;
...
$body$;
  ...
  execute 'revoke all on function public.kozmik_kaskad_settle_spin(uuid, text, bigint, uuid, jsonb, boolean) from public';
  execute 'revoke all on function public.kozmik_kaskad_settle_spin(uuid, text, bigint, uuid, jsonb, boolean) from anon';
  execute 'revoke all on function public.kozmik_kaskad_settle_spin(uuid, text, bigint, uuid, jsonb, boolean) from authenticated';
  execute 'grant execute on function public.kozmik_kaskad_settle_spin(uuid, text, bigint, uuid, jsonb, boolean) to service_role';
end;
$guard$;

-- ---------------------------------------------------------------------------
-- Match-3 oda girişi: admin coin ödemez
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_entry < coalesce((v_pol->>'min_entry')::bigint, 0) and not v_is_admin then
  ...
  if v_entry > 0 and not v_is_admin then
...
begin
  ...
  if exists (select 1 from public.game_session_players where session_id = p_session_id and user_id = v_uid) then
    ...
    if v_s.entry_amount > 0 and v_entry <> v_s.entry_amount and not v_is_admin then
